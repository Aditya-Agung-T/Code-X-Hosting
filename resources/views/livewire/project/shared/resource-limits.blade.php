<form wire:submit="submit" class="application-settings-form flex flex-col gap-6">
    ...
    @else
        <x-callout type="info" title="Managed Resource Quota">
            Resource specifications (RAM and CPU) for this container are configured and allocated by the hosting administrator according to your hosting plan.
        </x-callout>
    @endif
    <x-application.settings-section id="cpu-limits-section" title="CPU"
        ...
        <div class="grid gap-4 md:grid-cols-3">
            <x-forms.input :disabled="! $this->canEditLimits()" canGate="update" :canResource="$resource" placeholder="1.5"
                helper="Set to 0 to use all available CPUs. Decimal values such as 0.5 are supported."
                label="CPU limit" id="limitsCpus" />
            <x-forms.input :disabled="! $this->canEditLimits()" canGate="update" :canResource="$resource" placeholder="0-2"
            ...
            <x-forms.input :disabled="! $this->canEditLimits()" canGate="update" :canResource="$resource" placeholder="1024"
    ...
    <x-application.settings-section id="memory-limits-section" title="Memory"
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <x-forms.input :disabled="! $this->canEditLimits()" canGate="update" :canResource="$resource"
