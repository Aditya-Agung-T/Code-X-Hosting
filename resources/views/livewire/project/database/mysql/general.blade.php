<div class="application-settings-form">
    <form wire:submit="submit" class="flex flex-col gap-6">
        ...
        <x-application.settings-section title="Database details"
            description="Manage the identity and container image for this MySQL database.">
            <x-slot:actions>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="lg:col-span-2">
                    <x-forms.input label="Image" id="image" required canGate="update" :canResource="$database"
                        helper="Use a published MySQL image from Docker Hub." />
                </div>
            </div>
        </x-application.settings-section>

        <x-application.settings-section title="Credentials"
            description="Keep these values aligned with the credentials configured inside MySQL.">
            @if ($database->started_at)
                <x-callout type="warning" title="Keep credentials synchronized">
                    Changing values here does not update MySQL. Update MySQL first, then synchronize the values here so
            ...
            <div class="{{ $database->started_at ? 'mt-4 ' : '' }}grid gap-4 lg:grid-cols-2">
                ...
                @endif
                <x-forms.input label="Normal user" id="mysqlUser" required canGate="update"
        ...
        <x-application.settings-section title="Configuration"
            description="Override the MySQL configuration used by this container.">
            <x-forms.textarea label="Custom MySQL configuration" rows="10" id="mysqlConf"
                canGate="update" :canResource="$database" />
        </x-application.settings-section>
