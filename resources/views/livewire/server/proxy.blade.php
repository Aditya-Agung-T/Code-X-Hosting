    ...
    @if ($server->proxyType())
                                ...
                                @if ($proxySettings)
                                    <x-modal-confirmation title="Reset Proxy Configuration?"
                                        ...
                                        submitAction="resetProxyConfiguration" :actions="[
                                            'Reset the proxy configuration to Code X Hosting defaults.',
        ...
        @elseif($selectedProxy === 'NONE')
            <x-application.settings-section title="Custom proxy"
                helper="Code X Hosting will not manage a reverse proxy for this server.">
                <x-slot:actions>
                    @can('update', $server)
                ...
                <x-callout type="info" title="Custom proxy selected">
                    Configure and operate the proxy outside Code X Hosting.
                </x-callout>
            </x-application.settings-section>
        @else
            <x-application.settings-section title="Proxy configuration"
                ...
                @can('update', $server)
                    <div class="grid gap-3 lg:grid-cols-3">
                        @foreach ([
                            ['value' => 'NONE', 'title' => 'Custom', 'description' => 'Manage the proxy outside Code X Hosting.'],
                            ['value' => 'TRAEFIK', 'title' => 'Traefik', 'description' => 'Use the default Code X Hosting proxy.'],
                            ['value' => 'CADDY', 'title' => 'Caddy', 'description' => 'Use the Code X Hosting Caddy integration.'],
                        ] as $proxyOption)
